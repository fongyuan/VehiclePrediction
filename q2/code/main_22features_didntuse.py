...
if __name__ == "__main__":
    ...
    for i in range(num_train_files):
        ...
        for idx, name in np.ndenumerate(X.columns):
            if "role" in name and "agent" in X[name][0]:
                agent_pos = X.iloc[:,idx[0]+2:idx[0]+4]
                agent_pos = agent_pos.to_numpy().flatten()
                break

        X_train = np.append(X_train, agent_pos[np.newaxis,:], axis=0)
 ...
 ############
    ...
    #         X_val = np.append(X_val[:,2:],np.array([[pred_x, pred_y]]), axis=1)
