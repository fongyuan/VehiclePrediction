...
if __name__ == "__main__":
    ...
    X_train = np.empty((0,22*10)) # number of features
    ...
    for i in range(num_models):
        ...
        models_x.append(LM.Lasso(alpha=0.01).fit(X_train, y_train_x[:,i]))
        models_y.append(LM.Lasso(alpha=0.01).fit(X_train, y_train_y[:,i]))
        # models_x.append(Ridge().fit(X_train, y_train_x[:,i]))
        # models_y.append(Ridge().fit(X_train, y_train_y[:,i]))
 ...
 ############
    ...
    validation_error = 0
    X_val = np.empty((0,22*10)) # number of features
