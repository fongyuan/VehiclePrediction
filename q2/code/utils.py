...
# more recent point is last. the array should be of size 2
...
# more recent point is last. the array should be of size 3
def second_derivative(arr, h):
    assert arr.size == 3
    return (arr[2] - 2*arr[1] + arr[0])/(h**2)

# point 2 is the most recent point
def velocity(pt1, pt2, h):
    vx = derivative(np.array([pt1[0], pt2[0]]),h)
    vy = derivative(np.array([pt1[1], pt2[1]]),h)
    return np.array([vx,vy])

# point 3 is the most recent point
def acceleration(pt1, pt2, pt3, h):
    ax = second_derivative(np.array([pt1[0], pt2[0], pt3[0]]),h)
    ay = second_derivative(np.array([pt1[1], pt2[1], pt3[0]]),h)
